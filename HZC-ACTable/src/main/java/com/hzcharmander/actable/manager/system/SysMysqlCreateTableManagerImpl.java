package com.hzcharmander.actable.manager.system;
...
import com.hzcharmander.actable.annotation.LengthCount;
import com.hzcharmander.actable.annotation.Table;
import com.hzcharmander.actable.command.CreateTableParam;
import com.hzcharmander.actable.command.SysMysqlColumns;
import com.hzcharmander.actable.constants.MySqlTypeConstant;
import com.hzcharmander.actable.dao.system.CreateMysqlTablesMapper;
import com.hzcharmander.actable.utils.ClassTools;
...
public class SysMysqlCreateTableManagerImpl implements SysMysqlCreateTableManager {
    ...
    private void allTableMapConstruct(Map<String, Object> mySqlTypeAndLengthMap, Set<Class<?>> classes,
        ...
        for (Class<?> clas : classes) {
            ...
            String tableName = ("".equals(table.name()))?clas.getSimpleName():table.name();
            ...
            if (exist == 0) {
                ...
                buildAddAndRemoveAndModifyFields(mySqlTypeAndLengthMap, modifyTableMap, addTableMap, removeTableMap,
                        dropKeyTableMap, dropUniqueTableMap, tableName, newFieldList, removeFieldList, addFieldList,
    ...
    private void buildAddAndRemoveAndModifyFields(Map<String, Object> mySqlTypeAndLengthMap,
                                                  ...
                                                  Map<String, List<Object>> dropUniqueTableMap, String tableName, List<Object> newFieldList,
        ...
        buildNewFields(addTableMap, tableName, newFieldList, addFieldList, columnNames);
        ...
        // 2. 找出删除的字段
        buildRemoveFields(removeTableMap, tableName, removeFieldList, columnNames, fieldMap);

        // 3. 找出更新的字段
        buildModifyFields(mySqlTypeAndLengthMap, modifyTableMap, dropKeyTableMap, dropUniqueTableMap, tableName,
    ...
    private void buildModifyFields(Map<String, Object> mySqlTypeAndLengthMap, Map<String, List<Object>> modifyTableMap,
                                   Map<String, List<Object>> dropKeyTableMap, Map<String, List<Object>> dropUniqueTableMap, String tableName,
        ...
        if (dropKeyFieldList.size() > 0) {
            dropKeyTableMap.put(tableName, dropKeyFieldList);
        }

        if (dropUniqueFieldList.size() > 0) {
            dropUniqueTableMap.put(tableName, dropUniqueFieldList);
        }
    }
     ...
     */
    private void buildRemoveFields(Map<String, List<Object>> removeTableMap, String tableName, List<Object> removeFieldList,
     ...
     * @param columnNames  数据库中的结构
     */
    private void buildNewFields(Map<String, List<Object>> addTableMap, String tableName, List<Object> newFieldList,
    ...
    private void tableFieldsConstruct(Map<String, Object> mySqlTypeAndLengthMap, Class<?> clas,
        ...
        for (Field field : fields) {
            ...
            if (hasAnnotation) {
                ...
                String type = convertType(field, column.length());
                String name=convertName(field,column);
                ...
                int length = (Integer) mySqlTypeAndLengthMap.get(type);
    ...
    public String convertName(Field field,Column column){
    ...
    public String convertType(Field field, int length) {
        String type = MySqlTypeConstant.VARCHAR;
        String typeStr = field.getGenericType().toString().replace("class", "").trim();
        //根据状态值，判断
        if (typeStr.equals("java.lang.Integer")) {
            ...
            if (length == 1) {
                type = MySqlTypeConstant.CHAR;
            } else if (length > 255)
                type = MySqlTypeConstant.TEXT;
        } else if (typeStr.equals("java.sql.Timestamp")||typeStr.equals("java.util.Date")) {
            type = MySqlTypeConstant.DATETIME;
        } else if (typeStr.equals("java.math.BigDecimal")) {
            type = MySqlTypeConstant.DECIMAL;
        } else if (typeStr.equals("java.lang.Double")) {
            type = MySqlTypeConstant.DOUBLE;
        }
        return type;
    }
    ...
    private Field[] recursionParents(Class<?> clas, Field[] fields) {
        if (clas.getSuperclass() != null) {
            ...
            fields = (Field[]) ArrayUtils.addAll(clsSup.getDeclaredFields(),fields);
